...
<head>
    ...
    <title><?= esc($title ?? 'Edit Menu') ?></title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white shadow-sm px-8 py-4 flex items-center gap-4">
            ...
            <div>
                <h1 class="font-serif text-2xl font-bold text-banjar-dark">Edit Menu</h1>
                <p class="text-gray-500 text-sm"><?= esc($menu['nama_menu']) ?></p>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto p-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 max-w-2xl">
                <div class="p-8">
                    <form action="<?= base_url('/admin/update/' . $menu['id']) ?>" method="POST" enctype="multipart/form-data" novalidate>
                        <?= csrf_field() ?>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="kategori" class="input-focus w-full px-4 py-3 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-800 bg-white">
                                <?php foreach(['Soto', 'Lauk', 'Minuman', 'Dessert', 'Paket'] as $k): ?>
                                <option value="<?= $k ?>" <?= old('kategori', $menu['kategori']) === $k ? 'selected' : '' ?>><?= $k ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="deskripsi" rows="4"
                                class="input-focus w-full px-4 py-3 border-2 border-gray-200 rounded-xl outline-none transition-all text-gray-800 resize-none"><?= old('deskripsi', $menu['deskripsi']) ?></textarea>
                        </div>
                        ...
                        <!-- Gambar -->
                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Menu</label>
                            <?php if ($menu['gambar'] && file_exists(FCPATH . 'uploads/menu/' . $menu['gambar'])): ?>
                            <div class="mb-3 flex items-center gap-3">
                                <img src="<?= base_url('uploads/menu/' . $menu['gambar']) ?>" alt="gambar" class="h-20 w-20 object-cover rounded-xl shadow-md">
                                <span class="text-sm text-gray-500 italic">Gambar saat ini</span>
                            </div>
                            <?php endif; ?>
                            <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-banjar transition-colors cursor-pointer" onclick="document.getElementById('gambar').click()">
                                ...
                                <p class="text-gray-400 text-sm">Upload gambar baru (kosongkan jika tidak diubah)</p>
                            ...
                            <div id="img-preview" class="hidden mt-3">
                                <img id="preview-img" src="" class="max-h-40 rounded-xl object-cover shadow-md">
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <div class="relative">
                                    <input type="checkbox" name="is_available" value="1" <?= old('is_available', $menu['is_available']) ? 'checked' : '' ?> class="sr-only peer">
                                    <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-banjar transition-colors"></div>
                        ...
                        <div class="flex gap-3 pt-4 border-t border-gray-100">
                            <a href="<?= base_url('/admin') ?>" class="flex-1 text-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 rounded-xl transition-colors">Batal</a>
